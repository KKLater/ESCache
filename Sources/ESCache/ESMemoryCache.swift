...
public class ESMemoryCache {
    static let `default` = ESMemoryCache()
    var name: String
    ...
    func removeExpired() {
        ...
        for key in keys {
            ...
            guard let object = autoCache.object(forKey: nsKey) as? ESCacheModel else {
                keys.remove(key)
                continue
            }
...
extension ESMemoryCache: ESCacheable {
    ...
    public func exists(for key: String) -> Bool {
        lock.lock()
        guard let cacheModel: ESCacheModel = autoCache.object(forKey: key as NSString) as? ESCacheModel else {
            keys.remove(key)
            lock.unlock()
            return false
        }
